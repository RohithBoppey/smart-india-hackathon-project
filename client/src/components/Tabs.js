...
const Tabs = () => {
	...
	const submitHandler1 = async (event) => {
		...
		const temp = {
			...
			status_of_ground: reqOneInputFour_a.current.checked
				? "Construction"
				: "Renovation",
		};
	...
	const submit1 = (event) => {
		event.preventDefault();
		console.log(
			reqOneInputFour_a.current.checked ? "Construction" : "Renovation"
		);
		// console.log(reqOneInputFour_b.current.value)
	};
